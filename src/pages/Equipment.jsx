...
export default function Equipment({ user, onLogin }) {
  ...
  const [registerForm, setRegisterForm] = useState({ machineType: 'EQ-TRAC', machineName: '', location: '', biddingPrice: '' });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '580px' }}>
          <span style={{
            fontSize: '11px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase',
          ...
          <h1 style={{ fontSize: '24px', fontWeight: 800, color: '#111827', margin: '8px 0 4px', letterSpacing: '-0.5px' }}>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '10px',
              border: 'none',
              backgroundColor: activeTab === 'renter' ? '#22c55e' : 'transparent',
              color: activeTab === 'renter' ? '#fff' : '#6b7280',
              fontWeight: 600,
              fontSize: '12px',
              cursor: 'pointer',
      ...
      {activeTab === 'renter' && (
        <>
          <div style={{
            ...
            <h2 style={{ fontSize: '17px', fontWeight: 700, color: '#111827', margin: '0 0 16px', letterSpacing: '-0.3px' }}>
            ...
            <form onSubmit={handleSubmitRequest} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
              <div>
                ...
                <select
                  ...
                  style={{
                    ...
                    backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.14)',
              ...
              <div>
                ...
                >
                  {(WORK_TYPES_BY_EQUIPMENT[formData.equipmentTypeId] || DEFAULT_WORK_TYPES).map(w => (
                    <option key={w} value={w} style={{ backgroundColor: '#ffffff', color: '#111827' }}>
                      {w}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                <input
                  ...
                  style={{
                    width: '100%', padding: '10px 14px', borderRadius: '12px',
                    backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.14)',
                    color: '#111827', fontSize: '13px', outline: 'none', colorScheme: 'light'
                  }}
                  required
                />
              </div>
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid rgba(0,0,0,0.08)', paddingTop: '16px' }}>
                <div>
                  <span style={{ fontSize: '12px', color: '#6b7280' }}>Estimated Fixed Cost:</span>
                  <div style={{ fontSize: '18px', fontWeight: 700, color: '#15803d' }}>
                    ₹{estimatedCost} <span style={{ fontSize: '12px', color: '#6b7280', fontWeight: 400 }}>({formData.landAreaAcres} acres × ₹{selectedType.defaultRate}/acre)</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={submitting}
                  style={{
                    backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px',
                ...
                <div key={req.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.08)', paddingBottom: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ fontFamily: 'monospace', fontSize: '12px', fontWeight: 700, color: '#15803d' }}>{req.id}</span>
                      <span style={{ fontSize: '14px', fontWeight: 700, color: '#111827' }}>{req.equipmentTypeName} — {req.workType}</span>
                    </div>
                  ...
                  <div style={{ fontSize: '12px', color: '#6b7280', display: 'flex', gap: '20px' }}>
                    <span>Area: <strong style={{ color: '#111827' }}>{req.landAreaAcres} Acres</strong></span>
                    <span>Location: <strong style={{ color: '#111827' }}>{req.location}</strong></span>
                    <span>Required: <strong style={{ color: '#111827' }}>{req.requiredDate} ({req.preferredTime})</strong></span>
                  </div>
                  ...
                  {req.status === 'booked' && req.acceptedQuote && (
                    <div style={{ backgroundColor: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', padding: '14px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: '#15803d' }}>BOOKED & DISPATCHED</span>
                        <h4 style={{ fontSize: '14px', fontWeight: 700, color: '#111827', margin: '2px 0' }}>{req.acceptedQuote.machineName}</h4>
                        <p style={{ fontSize: '11px', color: '#6b7280', margin: 0 }}>Driver: {req.acceptedQuote.ownerName} via {req.acceptedQuote.shopName}</p>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '16px', fontWeight: 700, color: '#15803d' }}>₹{req.acceptedQuote.calculatedPrice}</div>
                        <span style={{ fontSize: '11px', color: '#2563eb' }}>Call: {req.acceptedQuote.shopPhone}</span>
                      </div>
                    </div>
                  )}
                  ...
                  {req.status !== 'booked' && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '10px' }}>
                      {(!req.quotes || req.quotes.length === 0) ? (
                        <div style={{ fontSize: '12px', color: '#666', padding: '12px', backgroundColor: 'rgba(0,0,0,0.03)', borderRadius: '10px', textAlign: 'center' }}>
                          Pings sent to nearby equipment owners. Waiting for an owner to accept...
                        </div>
                      ) : (
                        req.quotes.map(q => (
                          <div key={q.quoteId} style={{ backgroundColor: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                              <div style={{ fontSize: '13px', fontWeight: 700, color: '#111827' }}>{q.machineName}</div>
                              <div style={{ fontSize: '11px', color: '#6b7280' }}>Owner: {q.ownerName} ({q.distanceKm})</div>
                              <div style={{ fontSize: '11px', color: '#b45309', marginTop: '2px' }}>Rating: {q.rating} ★</div>
                            </div>
                            <div style={{ textAlign: 'right' }}>
                              <div style={{ fontSize: '15px', fontWeight: 700, color: '#15803d' }}>₹{q.calculatedPrice}</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', borderTop: '1px solid rgba(0,0,0,0.08)', paddingTop: '8px' }}>
                    {req.status === 'booked' && req.acceptedQuote && (
                      <button
                        onClick={() => navigate(
                        ...
                        style={{ backgroundColor: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)', color: '#2563eb', padding: '4px 10px', borderRadius: '8px', fontSize: '11px', cursor: 'pointer' }}
                      >
                        Track
                      </button>
                    )}
                    <button onClick={() => handleCancelRequest(req.id)} style={{ backgroundColor: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', color: '#b91c1c', padding: '4px 10px', borderRadius: '8px', fontSize: '11px', cursor: 'pointer' }}>
                      Cancel Request
                    </button>
                  </div>
                </div>
      ...
      {activeTab === 'owner' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {!user ? (
            <div style={{ backgroundColor: 'rgba(255,255,255,0.9)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '16px', padding: '20px', textAlign: 'center' }}>
              <p style={{ color: '#6b7280', fontSize: '13px', margin: '0 0 12px' }}>Log in to register your machine and start earning.</p>
              <button
                onClick={onLogin}
            ...
            <div style={{ backgroundColor: 'rgba(255,255,255,0.9)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: '16px', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div>
                <p style={{ color: '#111827', fontSize: '14px', fontWeight: 700, margin: 0 }}>You haven't registered a machine yet</p>
                <p style={{ color: '#6b7280', fontSize: '12px', margin: '4px 0 0' }}>Your machine's address must be within {OWNER_JOB_RADIUS_KM} km of your field location ({user.fieldLocation || 'not set'}).</p>
              </div>
              <button
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <label style={{ fontSize: '11px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase' }}>My Machines ({myOwners.length}/{MAX_MACHINES_PER_FARMER})</label>
              {myOwners.map(o => (
                <div key={o.id} style={{ backgroundColor: 'rgba(255,255,255,0.9)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '16px', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                  <div>
                    <div style={{ color: '#111827', fontSize: '14px', fontWeight: 700 }}>{o.machineName}</div>
                    <div style={{ color: '#6b7280', fontSize: '12px', marginTop: '2px' }}>{o.location}</div>
                  </div>
                  <button
                    onClick={() => handleToggleMyAvailability(o.id, o.available)}
                    style={{
                      backgroundColor: o.available ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)',
                      border: `1px solid ${o.available ? 'rgba(16,185,129,0.4)' : 'rgba(239,68,68,0.4)'}`,
                      color: o.available ? '#15803d' : '#b91c1c',
              ...
              {myOwners.length < MAX_MACHINES_PER_FARMER ? (
                <button
                  onClick={() => setShowRegisterModal(true)}
                  style={{ alignSelf: 'flex-start', backgroundColor: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.35)', color: '#15803d', borderRadius: '10px', padding: '8px 16px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}
                >
                  + Add Another Machine
                </button>
              ) : (
          ...
          <div>
            ...
            {(() => {
              ...
              if (nearbyRequests.length === 0) {
                return (
                  <div style={{ padding: '30px', backgroundColor: 'rgba(0,0,0,0.03)', borderRadius: '14px', color: '#666', textAlign: 'center' }}>
                ...
                nearbyRequests.map(req => {
                  ...
                  return (
                    <div key={req.id} style={{
                      ...
                      border: hasAlreadyAccepted ? '1px solid rgba(16,185,129,0.4)' : '1px solid rgba(34, 197, 94,0.3)',
                      borderRadius: '16px',
                      padding: '18px',
                      display: 'flex',
                      justifyContent: 'space-between',
                      ...
                      <div style={{ spaceY: '4px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ fontSize: '10px', fontWeight: 700, backgroundColor: '#22c55e', color: '#fff', padding: '2px 8px', borderRadius: '4px' }}>LIVE PING</span>
                          <span style={{ fontSize: '14px', fontWeight: 700, color: '#111827' }}>{req.equipmentTypeName} — {req.workType}</span>
                        </div>
                      ...
                      <div style={{ textAlign: 'right', minWidth: '160px' }}>
                        <div style={{ fontSize: '11px', color: '#6b7280' }}>Your Net Payout (90%):</div>
                        <div style={{ fontSize: '20px', fontWeight: 800, color: '#15803d' }}>₹{estimatedPayout}</div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'flex-end' }}>
                            {matchingMachines.length > 1 && (
                              <select
                                ...
                                style={{ backgroundColor: '#ffffff', color: '#111827', border: '1px solid rgba(0,0,0,0.14)', padding: '4px 8px', borderRadius: '8px', fontSize: '11px', outline: 'none' }}
                              >
      ...
      {showRegisterModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setShowRegisterModal(false)}>
          <div style={{ backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '20px', padding: '28px', maxWidth: '440px', width: '100%' }} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ color: '#111827', fontSize: '18px', fontWeight: 700, margin: '0 0 4px' }}>Register as Machine Owner</h2>
            ...
            <form onSubmit={handleRegisterOwner} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                <label style={{ fontSize: '11px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Pick a Preset (optional)</label>
                <select
                  onChange={(e) => {
                  ...
                  style={{ width: '100%', backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.14)', borderRadius: '10px', padding: '11px 14px', color: '#111827', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
                >
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Machine Name & Model</label>
                <input
                  type="text"
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Address / Location</label>
                <input
                  type="text"
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Bidding Price (₹)</label>
                <input
                  type="number"
                  value={registerForm.biddingPrice}
                  onChange={(e) => setRegisterForm({ ...registerForm, biddingPrice: e.target.value })}
                  required
                  style={{ width: '100%', backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.14)', borderRadius: '10px', padding: '11px 14px', color: '#111827', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
                />
                <p style={{ fontSize: '11px', color: '#6b7280', marginTop: '4px' }}>The price you charge per acre. Set it low to win more jobs.</p>
              </div>

              {registerError && (
                <div style={{ backgroundColor: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '8px', padding: '10px 14px', color: '#b91c1c', fontSize: '12px' }}>
                  {registerError}
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                <button
                  ...
                  style={{ flex: 1, padding: '11px', backgroundColor: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '10px', color: '#6b7280', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
                >
                  Cancel
                </button>
                <button
