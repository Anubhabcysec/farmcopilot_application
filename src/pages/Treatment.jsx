...
export default function Treatment() {
  ...
  const renderStars = (rating) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
      {[...Array(5)].map((_, i) => (
        <span key={i} style={{ fontSize: '11px', color: i < Math.floor(rating) ? '#eab308' : 'rgba(0,0,0,0.12)' }}>★</span>
      ))}
  ...
  return (
    <div style={{ maxWidth: '960px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '40px', paddingBottom: '60px' }}>
      ...
      <div style={{
        ...
        onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(34, 197, 94,0.35)'; e.currentTarget.style.boxShadow = '0 0 40px rgba(34, 197, 94,0.1)'; }}
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
          ...
          <span style={{ marginLeft: 'auto', fontSize: '11px', color: '#6b7280', fontFamily: 'monospace', letterSpacing: '0.5px' }}>treatment-finder</span>
        </div>
        ...
        <div ref={dropdownRef} style={{ position: 'relative' }}>
          <div
            ...
            style={{
              ...
              borderRadius: '10px',
              padding: '12px 16px',
              border: `1px solid ${diseaseFreeText ? 'rgba(34,197,94,0.3)' : dropdownOpen ? 'rgba(34, 197, 94,0.4)' : 'rgba(0,0,0,0.1)'}`,
              display: 'flex',
          ...
          >
            ...
            {diseaseFreeText ? (
              <button
                ...
                style={{ background: 'none', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '6px', color: '#666', fontSize: '11px', padding: '2px 8px', cursor: 'pointer' }}
              >✕</button>
            ) : (
          ...
          {dropdownOpen && (
            <div style={{
              ...
              backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '12px',
              boxShadow: '0 12px 48px rgba(0,0,0,0.18)', maxHeight: '240px', overflowY: 'auto', padding: '4px 0',
            }}>
              {diseases.map(d => (
                <div
                  key={d.key}
                  ...
                  style={{
                    ...
                    color: selectedDisease === d.key ? '#1f2937' : '#6b7280',
                    backgroundColor: selectedDisease === d.key ? 'rgba(0,0,0,0.04)' : 'transparent',
        ...
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[1, 2].map(i => (
              <div key={i} style={{
                backgroundColor: 'rgba(0,0,0,0.03)', borderRadius: '10px',
        ...
        {advisoryProducts.length > 0 && (
          <div style={{
            backgroundColor: 'rgba(34, 197, 94,0.06)', borderRadius: '12px',
            padding: '14px 16px', border: '1px solid rgba(34, 197, 94,0.2)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {advisoryProducts.map(prod => (
                <div key={prod.id} style={{
                  ...
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#1f2937' }}>{prod.name}</div>
                    <div style={{ fontSize: '11px', color: '#6b7280', fontStyle: 'italic', marginBottom: '4px' }}>{prod.activeIngredient}</div>
                    {prod.dosage && <div style={{ fontSize: '11px', color: '#6b7280' }}>Dose: {prod.dosage}</div>}
                    <button
                      type="button"
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ fontSize: '14px', fontWeight: 700, color: '#15803d' }}>₹{prod.price}</span>
                    <div style={{
                      fontSize: '9px', fontWeight: 700, color: '#15803d', textTransform: 'uppercase',
        ...
        {result && result.found && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <div style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(0,0,0,0.15)'; e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.05)'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(0,0,0,0.08)'; e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.03)'; }}
            >
              <div>
                <div style={{ fontSize: '14px', fontWeight: 600, color: '#1f2937', marginBottom: '3px' }}>{result.pesticide}</div>
                <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px' }}>Dose: {result.dosage}</div>
                <button
                  type="button"
                  onClick={() => setSelectedInstruction({
            ...
            {result.alternative && (
              ...
              >
                <div>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#1f2937', marginBottom: '3px' }}>{result.alternative}</div>
                  <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px' }}>Dose: {result.altDosage}</div>
                  <button
                    type="button"
                    onClick={() => setSelectedInstruction({
                ...
                <span style={{
                  ...
                  backgroundColor: 'rgba(34, 197, 94,0.08)', border: '1px solid rgba(34, 197, 94,0.18)',
                  padding: '3px 10px', borderRadius: '100px', textTransform: 'capitalize',
                }}>{result.category}</span>
              </div>
            )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#111827', margin: 0, letterSpacing: '-0.3px' }}>
              {equipTrack
                ? (equipArrived ? `Arrived — ${equipTrack.label}` : `Tracking ${equipTrack.id} — ${equipTrack.label}`)
                : 'Nearby Agri Shops'}
            </h2>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={() => fetchNearbyShops(true)} style={{
              ...
              color: '#2563eb', fontSize: '12px', fontWeight: 500, padding: '6px 14px', cursor: 'pointer',
              transition: 'all 0.2s',
            }}
            ...
            <button
              ...
              style={{
                ...
                border: nearestTenOnly ? '1px solid rgba(74,222,128,0.4)' : '1px solid rgba(0,0,0,0.1)',
                borderRadius: '8px',
                color: nearestTenOnly ? '#15803d' : '#6b7280',
          ...
          <div style={{ flex: '1 1 400px', minHeight: '500px', ...glassPanel, padding: '6px' }}>
            {!isLoaded ? (
              <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '500px' }}>
                ...
                <p style={{ color: '#6b7280', marginTop: '12px', fontSize: '13px' }}>Loading Map…</p>
              </div>
            ) : (
                ...
                <GoogleMap
                  ...
                  options={{
                    styles: [
                      ...
                      { featureType: "poi.park", elementType: "labels.text.fill", stylers: [{ color: "#6b8f5f" }] },
                ...
                {equipTrack && routeInfo && (
                  <div style={{ position: 'absolute', bottom: '40px', left: '12px', zIndex: 10, width: '240px' }}>
                    <div style={{
                      ...
                      padding: '14px', boxShadow: '0 12px 40px rgba(0,0,0,0.18)',
                    }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
                        <div style={{ width: '7px', height: '7px', borderRadius: '50%', backgroundColor: equipArrived ? '#15803d' : '#f97316', animation: 'pulse 2s infinite' }} />
                        <span style={{ fontSize: '9px', fontWeight: 700, color: equipArrived ? '#15803d' : '#c2410c', letterSpacing: '0.6px' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: '40px', left: '12px', zIndex: 10, width: '240px' }}>
                    <div style={{
                      ...
                      <button onClick={() => openInGoogleMaps(selectedShop)} style={{
                        ...
                        padding: '8px', backgroundColor: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)',
                        borderRadius: '10px', color: '#2563eb', fontSize: '11px', fontWeight: 600, cursor: 'pointer',
            ...
            {shopLoading ? (
              ...
              displayedShops.map((shop, i) => (
                <div
                  ...
                  style={{
                    backgroundColor: selectedShop?.name === shop.name ? 'rgba(59,130,246,0.06)' : '#ffffff',
                    border: `1px solid ${selectedShop?.name === shop.name ? 'rgba(59,130,246,0.25)' : 'rgba(0,0,0,0.08)'}`,
                    borderRadius: '14px', padding: '16px', cursor: 'pointer',
                    transition: 'all 0.2s ease',
                  }}
                  onMouseEnter={e => { if (selectedShop?.name !== shop.name) { e.currentTarget.style.borderColor = 'rgba(0,0,0,0.15)'; e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.02)'; } }}
                  onMouseLeave={e => { if (selectedShop?.name !== shop.name) { e.currentTarget.style.borderColor = 'rgba(0,0,0,0.08)'; e.currentTarget.style.backgroundColor = '#ffffff'; } }}
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <h4 style={{ fontSize: '13px', fontWeight: 600, color: '#111827', margin: 0 }}>{shop.name}</h4>
                    {shop.availability && (
                      <span style={{
                        fontSize: '9px', fontWeight: 600, padding: '2px 8px', borderRadius: '100px',
                        color: shop.availability === 'In Stock' ? '#15803d' : shop.availability === 'Limited Stock' ? '#a16207' : '#6b7280',
                        backgroundColor: shop.availability === 'In Stock' ? 'rgba(74,222,128,0.12)' : shop.availability === 'Limited Stock' ? 'rgba(250,204,21,0.15)' : 'rgba(0,0,0,0.04)',
                        border: `1px solid ${shop.availability === 'In Stock' ? 'rgba(74,222,128,0.25)' : shop.availability === 'Limited Stock' ? 'rgba(250,204,21,0.3)' : 'rgba(0,0,0,0.1)'}`,
                        flexShrink: 0,
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {shop.phone && (
                      <a href={`tel:${shop.phone.replace(/\s+/g, '')}`} onClick={e => e.stopPropagation()} style={{
                        ...
                        padding: '7px', backgroundColor: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.1)',
                        borderRadius: '8px', color: '#374151', fontSize: '11px', fontWeight: 500, textDecoration: 'none',
                        transition: 'all 0.2s',
                      }}
                    ...
                    <button onClick={e => { e.stopPropagation(); openInGoogleMaps(shop); }} style={{
                      ...
                      borderRadius: '8px', color: '#2563eb', fontSize: '11px', fontWeight: 500, cursor: 'pointer',
                      transition: 'all 0.2s',
                    }}
              ...
              <div style={{ ...glassPanel, padding: '24px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>No stores found nearby.</div>
            )}
          </div>
        </div>
      </div>
