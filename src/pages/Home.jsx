...
const ALL_STYLES = `
  ...
  @keyframes glowPulse {
    0%, 100% { box-shadow: 0 0 32px rgba(34, 197, 94,0.25); }
    50%  { box-shadow: 0 0 52px rgba(34, 197, 94,0.55), 0 0 80px rgba(74, 222, 128,0.2); }
  }
...
const FEATURE_ACCENTS = {
  advisory: { c: '#22c55e', text: '#15803d', rgb: '34,197,94' },
  treatment: { c: '#6366f1', text: '#4338ca', rgb: '99,102,241' },
  weather: { c: '#10b981', text: '#047857', rgb: '16,185,129' },
};
const AMBER_ACCENT = { c: '#f59e0b', text: '#b45309', rgb: '245,158,11' };
        ...
        {HERO_PALETTES.map((p, i) => {
          ...
          return (
            <div key={p.key} style={{
              ...
              backgroundColor: 'rgba(9,41,24,0.94)',
              backdropFilter: 'blur(16px)',
              WebkitBackdropFilter: 'blur(16px)',
              borderColor: isFront ? `rgba(${accent.rgb},0.45)` : 'rgba(134,239,172,0.22)',
              zIndex: 3 - rank,
              ...
              boxShadow: isFront
                ? `0 30px 70px rgba(3,20,10,0.4), 0 0 46px rgba(${accent.rgb},0.2), 0 0 0 1px rgba(134,239,172,0.12) inset`
                : '0 14px 34px rgba(3,20,10,0.3)',
            }}>
  ...
  return (
    <div style={panelBg}>
      ...
      <div style={{
        ...
        // A real photographed/rendered image, not a flat CSS color — some
        // browser "dark mode for web content" settings and color-inverting
        // extensions (Dark Reader and similar) recolor flat light
        // backgrounds but leave actual raster images alone, which is what
        // was turning this box dark brown even after the site opted out
        // of forced dark mode via `color-scheme`. A real light-beige photo
        // here renders as light beige everywhere, no exceptions.
        backgroundImage: `url(${softBeigeBg})`,
        ...
        {isPlaying && !scanDone && (
          <div style={{
            position: 'absolute', inset: 0,
            background: 'linear-gradient(90deg, transparent, rgba(21,128,61,0.35), transparent)',
        ...
        {scanDone && (
          <div style={{
            ...
            fontSize: '10px', fontWeight: 700, color: '#14532d',
            backgroundColor: 'rgba(255,255,255,0.55)', padding: '2px 8px', borderRadius: '100px',
            border: '1px solid rgba(21,128,61,0.35)', textShadow: 'none',
          }}>Analyzed ✓</div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '13px', color: '#e2e8f0', fontWeight: 500, minHeight: '18px' }}>
          ...
          {isPlaying && typed.length < FULL_TEXT.length && (
            <span style={{ animation: 'sc-blinkCursor 0.7s infinite', borderRight: '2px solid #4ade80', marginLeft: '1px' }} />
          )}
        </div>
      </div>
      ...
      <div style={{ opacity: confVisible ? 1 : 0, transition: 'opacity 0.4s' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#555', marginBottom: '5px' }}>
          <span>Confidence Score</span><span style={{ color: '#4ade80' }}>85%</span>
        </div>
...
function TreatmentAnimation({ isPlaying, onComplete }) {
  ...
  const products = [
    { name: 'Tricyclazole 75% WP', dose: '0.6 g/L', type: 'Fungicide', color: '#4ade80' },
  ...
  return (
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      <div style={{
        ...
        padding: '10px 14px', border: '1px solid rgba(34, 197, 94,0.3)',
    ...
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div>
        ...
        <div style={{
          backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: '8px',
          padding: '10px 14px', border: '1px solid rgba(34, 197, 94,0.3)',
          fontFamily: 'monospace', fontSize: '15px', fontWeight: 600, color: '#4ade80',
          letterSpacing: '2px', minHeight: '42px', display: 'flex', alignItems: 'center',
        }}>
          {typedCode}
          {isPlaying && typedCode.length < FULL_CODE.length && (
            <span style={{ borderRight: '2px solid #4ade80', animation: 'sc-blinkCursor 0.7s infinite', height: '16px', display: 'inline-block', marginLeft: '2px' }} />
          )}
        </div>
      </div>
...
function ProductsAnimation({ isPlaying }) {
  ...
  const products = [
    ...
    { name: 'Mancozeb 75% WP', cat: 'Fungicide', price: '₹260/kg', rating: '4.7', catColor: '#6ee7b7', catBg: 'rgba(110, 231, 183,0.1)' },
  ];
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        {['All', 'Fertilizer', 'Pesticide', 'Fungicide'].map((c, i) => (
          <span key={c} style={{
            ...
            backgroundColor: i === 0 ? 'rgba(34, 197, 94,0.15)' : 'rgba(255,255,255,0.04)',
            border: `1px solid ${i === 0 ? 'rgba(34, 197, 94,0.3)' : 'rgba(255,255,255,0.07)'}`,
            color: i === 0 ? '#4ade80' : '#555',
          }}>{c}</span>
        ))}
      </div>
...
═══════════════════════════════════════════ */
function ShowcaseRow({ animLeft, title, tag, desc, path, icon, AnimComponent, navigate, accent, stacked }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
        <span
          style={{
            ...
            backgroundColor: `rgba(${rgb},0.14)`, border: `1px solid rgba(${rgb},0.3)`,
            transition: 'transform 0.2s ease, background-color 0.2s ease, border-color 0.2s ease',
            cursor: 'pointer',
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.transform = 'scale(1.08) translateY(-1px)';
            e.currentTarget.style.backgroundColor = `rgba(${rgb},0.26)`;
            e.currentTarget.style.borderColor = `rgba(${rgb},0.55)`;
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.transform = 'scale(1) translateY(0)';
            e.currentTarget.style.backgroundColor = `rgba(${rgb},0.14)`;
        ...
        <h3 style={{
          ...
          letterSpacing: '-0.8px', color: '#111827', margin: 0, lineHeight: 1.1,
        }}>{title}</h3>
      </div>
...
export default function Home() {
  ...
  const growthZoneRef = useRef(null);
  ...
  return (
    <div style={{ paddingTop: '56px', color: '#111827', fontFamily: "'Inter', sans-serif" }}>
      ...
      <section style={{
        ...
        {/* Real farm photography + three.js ambient layer — scoped to just
            this hero section (not the whole site). aria-hidden and
            pointer-events:none, so it never touches any button or link. */}
        <FarmBackdrop />

        {/* Floating gradient orbs */}
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '-80px', left: '-120px', width: '420px', height: '420px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(34, 197, 94,0.18) 0%, transparent 70%)', animation: 'orbFloat1 9s ease-in-out infinite', filter: 'blur(2px)' }} />
          <div style={{ position: 'absolute', top: '-60px', right: '-100px', width: '360px', height: '360px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(74, 222, 128,0.14) 0%, transparent 70%)', animation: 'orbFloat2 11s ease-in-out infinite', filter: 'blur(2px)' }} />
          <div style={{ position: 'absolute', bottom: '-100px', left: '50%', transform: 'translateX(-50%)', width: '500px', height: '240px', borderRadius: '50%', background: 'radial-gradient(ellipse, rgba(21, 128, 61,0.12) 0%, transparent 70%)', animation: 'orbFloat3 13s ease-in-out infinite', filter: 'blur(8px)' }} />
        </div>
        {/* Dot-grid */}
        ...
        <div style={{ textAlign: 'center', maxWidth: '620px', margin: '0 auto', width: '100%' }}>
          <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'center', animation: 'badgePop 0.7s cubic-bezier(0.34,1.56,0.64,1) 0.1s both' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 14px', borderRadius: '100px', border: `1px solid ${aiStatus === 'online' ? 'rgba(52,211,153,0.35)' : 'rgba(255,255,255,0.18)'}`, backgroundColor: aiStatus === 'online' ? 'rgba(6,20,12,0.6)' : 'rgba(6,10,8,0.55)', fontSize: '12px', fontWeight: 500, color: aiStatus === 'online' ? '#6ee7b7' : '#d4d4d4', letterSpacing: '0.2px', backdropFilter: 'blur(8px)' }}>
          ...
          <p style={{ fontSize: '18px', color: '#e5e7eb', lineHeight: 1.7, maxWidth: '560px', margin: '0 auto 40px', animation: 'fadeSlideUp 0.75s cubic-bezier(0.22,1,0.36,1) 0.5s both' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', animation: 'fadeSlideUp 0.75s cubic-bezier(0.22,1,0.36,1) 0.68s both' }}>
            <button className="hero-btn-primary" onClick={() => navigate('/advisory')} style={{ backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '10px', padding: '13px 28px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.25s cubic-bezier(0.22,1,0.36,1)' }}>
            ...
            return (
              <TiltCard key={s.value} maxDeg={7} scale={1.03} style={{ backgroundColor: '#ffffff', padding: 'clamp(18px, 4vw, 24px) clamp(14px, 3vw, 20px)', textAlign: 'center' }}>
                <div
                  ...
                  style={{
                    width: '36px', height: '36px', borderRadius: '8px', margin: '0 auto 10px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    backgroundColor: `rgba(${rgb},0.16)`, border: `1px solid rgba(${rgb},0.3)`,
                    transition: 'transform 0.2s ease, background-color 0.2s ease, border-color 0.2s ease',
                    cursor: 'pointer',
                  }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.transform = 'scale(1.1) translateY(-1px)';
                    e.currentTarget.style.backgroundColor = `rgba(${rgb},0.28)`;
                    e.currentTarget.style.borderColor = `rgba(${rgb},0.55)`;
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.transform = 'scale(1) translateY(0)';
                    e.currentTarget.style.backgroundColor = `rgba(${rgb},0.16)`;
                    e.currentTarget.style.borderColor = `rgba(${rgb},0.3)`;
                  }}
                >
      ...
      <section style={{ padding: 'clamp(48px, 10vw, 80px) clamp(20px, 5vw, 48px)', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 'clamp(32px, 6vw, 48px)' }}>
          <p style={{ fontSize: '12px', fontWeight: 600, color: '#15803d', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '12px' }}>
            Everything in one platform
          </p>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 800, letterSpacing: '-1px', color: '#111827', margin: '0 0 16px', lineHeight: 1.1 }}>
            Every tool a modern farmer needs
          </h2>
          <p style={{ fontSize: '16px', color: '#6b7280', maxWidth: '500px', margin: '0 auto 8px', lineHeight: 1.6 }}>
            Hover any preview to see it come alive — each feature, animated in real time.
          </p>
          <p style={{ fontSize: '11px', fontWeight: 600, color: '#15803d', letterSpacing: '1.2px', textTransform: 'uppercase', margin: '24px 0 0' }}>How it works</p>
          <h3 style={{ fontSize: 'clamp(18px, 3vw, 22px)', fontWeight: 700, letterSpacing: '-0.3px', color: '#111827', margin: '6px 0 0' }}>Get answers in seconds</h3>
        </div>

        {(() => {
          const flankItems = [
            ...showcaseFeatures.map((feature) => ({ type: 'feature', data: feature })),
            ...howItWorks.map((step) => ({ type: 'step', data: step })),
          ];
          const leftItems = flankItems.filter((_, i) => i % 2 === 0);
          const rightItems = flankItems.filter((_, i) => i % 2 === 1);
          ...
          const renderItem = (item) => {
            if (item.type === 'feature') {
              ...
              return (
                <ShowcaseRow
                  ...
                  path={feature.path}
                  icon={feature.icon}
                  AnimComponent={feature.Anim}
                  navigate={navigate}
                  accent={accentFor(feature.accentKey)}
                />
              );
            }
            ...
            return (
              <TiltCard key={step.step} maxDeg={4} scale={1.01} style={{ padding: '22px 24px', backgroundColor: '#ffffff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '14px' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '9px', backgroundColor: `rgba(${rgb},0.12)`, border: `1px solid rgba(${rgb},0.22)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: text, marginBottom: '14px', letterSpacing: '0.5px' }}>{step.step}</div>
                <h4 style={{ fontSize: '16px', fontWeight: 600, color: '#111827', margin: '0 0 6px', letterSpacing: '-0.2px' }}>{step.title}</h4>
                <p style={{ fontSize: '13px', color: '#6b7280', lineHeight: 1.6, margin: 0 }}>{step.desc}</p>
              </TiltCard>
            );
          };

          return (
            <div ref={growthZoneRef} className="growth-flank-row">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(28px, 6vw, 44px)' }}>
                {leftItems.map(renderItem)}
              </div>
              <div>
                <ScrollGrowthPlant zoneRef={growthZoneRef} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(28px, 6vw, 44px)' }}>
      ...
      {/* ── DIVIDER ── */}
      <div style={{ width: '100%', height: '1px', backgroundColor: 'rgba(0,0,0,0.08)', marginTop: '40px' }} />
      ...
      <section style={{ padding: '0 clamp(20px, 5vw, 32px) 96px', maxWidth: '1100px', margin: '0 auto', position: 'relative' }}>
        <div aria-hidden="true" style={{
          ...
          <div style={{ position: 'absolute', top: '-30px', left: '10%', width: '280px', height: '280px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(34, 197, 94,0.18) 0%, transparent 70%)', filter: 'blur(4px)' }} />
        ...
        <TiltCard maxDeg={4} scale={1.008} style={{ background: 'linear-gradient(135deg, rgba(34, 197, 94,0.14) 0%, rgba(99,102,241,0.08) 55%, rgba(16,185,129,0.05) 100%), #ffffff', border: '1px solid rgba(34, 197, 94,0.22)', borderRadius: '20px', padding: 'clamp(40px, 8vw, 56px) clamp(24px, 6vw, 48px)', textAlign: 'center', boxShadow: '0 30px 60px rgba(0,0,0,0.08), 0 0 60px rgba(34, 197, 94,0.06)' }}>
          <h2 style={{ fontSize: 'clamp(24px, 4vw, 32px)', fontWeight: 700, color: '#111827', letterSpacing: '-0.5px', margin: '0 0 12px' }}>Start protecting your crops today</h2>
          <p style={{ fontSize: '15px', color: '#6b7280', margin: '0 0 32px', maxWidth: '420px', display: 'block', marginLeft: 'auto', marginRight: 'auto' }}>
            Free to use. No account needed. Get your first AI crop diagnosis in under 30 seconds.
          </p>
          <button
            onClick={() => navigate('/advisory')}
            style={{ backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '10px', padding: '13px 32px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s' }}
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop: '1px solid rgba(0,0,0,0.08)', padding: '32px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>
