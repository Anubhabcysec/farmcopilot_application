    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {notice && (
        <div style={{
          ...
          borderRadius: '10px', padding: '10px 14px', color: '#15803d', fontSize: '13px',
        }}>✓ {notice}</div>
      )}
      {error && !showManual && (
        <div style={{
          background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)',
          borderRadius: '10px', padding: '10px 14px', color: '#b91c1c', fontSize: '13px',
        }}>{error}</div>
      )}
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          >
            {farms.map(f => <option key={f.id} value={f.id} style={{ background: '#ffffff' }}>{f.name}</option>)}
          </select>
          <Link to="/farms" style={{ fontSize: '12px', color: '#15803d', fontWeight: 600, textDecoration: 'none' }}>
            Manage farms →
          </Link>
        </div>
        {selectedFarm && (
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
          ...
          {connected ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: streaming ? '#15803d' : '#b45309' }}>
                <span style={{
                  width: '8px', height: '8px', borderRadius: '50%',
                  background: streaming ? '#22c55e' : '#f59e0b',
                  boxShadow: `0 0 8px ${streaming ? '#22c55e' : '#f59e0b'}`,
                }} />
                {streaming ? 'Live' : 'Idle'}
              </span>
              <button
                onClick={disconnect}
                style={{
                  padding: '8px 16px', background: 'none', border: '1px solid rgba(0,0,0,0.12)',
                  borderRadius: '8px', color: '#6b7280', fontSize: '12px', fontWeight: 600, cursor: 'pointer',
                }}
              >
                Disconnect
              </button>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              >
                {[9600, 19200, 38400, 57600, 115200].map(b => (
                  <option key={b} value={b} style={{ background: '#ffffff' }}>{b} baud</option>
                ))}
              </select>
              <button
                onClick={connect}
                ...
                style={{
                  padding: '10px 18px',
                  background: (connecting || !serialSupported) ? 'rgba(34, 197, 94,0.4)' : 'linear-gradient(135deg, #16a34a, #15803d)',
        ...
        {!serialSupported && (
          <p style={{
            marginTop: '12px', marginBottom: 0, fontSize: '12px', color: '#b45309',
        ...
        {serialError && (
          <p style={{ marginTop: '12px', marginBottom: 0, fontSize: '12px', color: '#b91c1c' }}>{serialError}</p>
        )}
      </div>
      ...
      {connected && (
        <div style={cardStyle}>
              ...
              return (
                <div key={key} style={{
                  ...
                  <div style={{ fontSize: '20px', fontWeight: 700, color: `rgb(${meta.rgb})`, letterSpacing: '-0.5px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: '#6b7280', fontWeight: 500, marginLeft: '4px' }}>{meta.unit}</span>
                  </div>
                </div>
              );
            })}
          ...
          {/* Stability bar */}
          <div style={{
            background: isStable ? 'rgba(16,185,129,0.06)' : 'rgba(0,0,0,0.02)',
            border: `1px solid ${isStable ? 'rgba(16,185,129,0.3)' : 'rgba(0,0,0,0.08)'}`,
            borderRadius: '12px', padding: '14px 16px',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px', gap: '10px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '13px', fontWeight: 600, color: (captured || isStable) ? '#15803d' : '#374151' }}>
                {captured
                  ? '✓ Reading captured'
            ...
            <div style={{ height: '6px', background: 'rgba(0,0,0,0.08)', borderRadius: '100px', overflow: 'hidden' }}>
              <div style={{
                ...
                background: isStable ? 'linear-gradient(90deg,#10b981,#34d399)' : 'linear-gradient(90deg,#16a34a,#4ade80)',
            ...
            {!captured && breach && (
              <p style={{ fontSize: '11px', color: '#b45309', margin: '10px 0 0', lineHeight: 1.6 }}>
            ...
            {captured ? (
              <div style={{ marginTop: '14px' }}>
                <p style={{ fontSize: '12px', color: '#15803d', margin: '0 0 12px', lineHeight: 1.6 }}>
                  {saving
                    ? 'Saving this reading…'
                    : `Values above are the average of the steady window — saved to ${selectedFarm?.name}. The meter is no longer being read.`}
                </p>
                <button
                  ...
                  style={{
                    ...
                    background: saving ? 'rgba(34, 197, 94,0.35)' : 'linear-gradient(135deg, #16a34a, #15803d)',
              ...
              <p style={{ fontSize: '12px', color: '#6b7280', margin: '14px 0 0', lineHeight: 1.6, textAlign: 'center' }}>
          ...
          <div style={{ marginTop: '12px' }}>
            <button
              ...
              style={{ background: 'none', border: 'none', color: '#6b7280', fontSize: '11px', cursor: 'pointer', padding: 0 }}
            >
            ...
            {showRaw && (
              <pre style={{
                ...
                borderRadius: '8px', color: '#2563eb', fontSize: '11px',
      ...
      <div style={cardStyle}>
        <h3 style={{ color: '#111827', fontSize: '15px', fontWeight: 700, margin: '0 0 14px' }}>
        ...
        {tests.length === 0 ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', minWidth: '620px' }}>
              ...
              <tbody>
                {tests.map((t) => (
                  <tr key={t.id} style={{ borderTop: '1px solid rgba(0,0,0,0.08)', color: '#374151' }}>
                    ...
                    <td style={{ padding: '10px', color: '#6b7280' }}>{t.crop || '—'}</td>
                    <td style={{ padding: '10px' }}>
                      <span style={{
                        fontSize: '10px', fontWeight: 600, borderRadius: '100px', padding: '2px 8px',
                        color: t.source === 'device' ? '#15803d' : '#b45309',
        ...
        <p style={{ color: '#6b7280', fontSize: '11px', margin: '14px 0 0' }}>
          AI analysis, trends and your farm's soil profile come next — these saved tests are what they will read.
        </p>
      </div>
      ...
      {showManual && (
        <>
          <div
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 999, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(6px)' }}
          />
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#111827', margin: 0 }}>Enter Soil Values</h2>
              <button
                onClick={() => setShowManual(false)}
                style={{ background: 'none', border: 'none', color: '#6b7280', fontSize: '18px', cursor: 'pointer', lineHeight: 1 }}
              >×</button>
            </div>
            <p style={{ color: '#6b7280', fontSize: '12px', margin: '0 0 18px' }}>
              Saved for <strong style={{ color: '#374151' }}>{selectedFarm?.name}</strong> and marked as a manual entry.
            </p>
            ...
            <form onSubmit={handleManualSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <button
                ...
                style={{
                  padding: '9px', background: 'rgba(0,0,0,0.03)',
